...
class RuleResourceTest {
    ...
    @Test
    @SuppressWarnings("unchecked")
    void testUpdateRulesRobinhoodRouting() {
        // Arrange
        RuleResource resource = spy(new RuleResource());
        Emitter<TrackingCommand> cardanoEmitter = mock(Emitter.class);
        Emitter<TrackingCommand> robinhoodEmitter = mock(Emitter.class);
        resource.trackingEmitter = cardanoEmitter;
        resource.robinhoodTrackingEmitter = robinhoodEmitter;

        when(cardanoEmitter.send(any(TrackingCommand.class))).thenReturn(CompletableFuture.completedFuture(null));
        when(robinhoodEmitter.send(any(TrackingCommand.class))).thenReturn(CompletableFuture.completedFuture(null));
        ...
        // Existing Robinhood rule to be removed
        GuildRoleRule existingRule = mock(GuildRoleRule.class);
        existingRule.id = 50L;
        existingRule.guildId = "123456";
        existingRule.roleId = "evm-role-old";
        existingRule.policyId = "0x1111111111111111111111111111111111111111";
        existingRule.chain = dk.panos.promofacie.db.Chain.ROBINHOOD;
        when(existingRule.getResolvedChain()).thenReturn(dk.panos.promofacie.db.Chain.ROBINHOOD);

        doReturn(List.of(existingRule)).when(resource).getExistingRules("123456");
        doReturn(0L).when(resource).countPolicyInOtherGuilds("0x1111111111111111111111111111111111111111", "123456");
        doReturn(0L).when(resource).countPolicyGlobally("0x2222222222222222222222222222222222222222");
        doNothing().when(resource).deletePendingEvaluations(50L);
        doNothing().when(resource).deleteRule(existingRule);
        doNothing().when(resource).persistRule(any(GuildRoleRule.class));
        doNothing().when(resource).persistPendingEvaluation(any(PendingRuleEvaluation.class));
        doNothing().when(resource).flushSession();

        // One Cardano wallet and one Robinhood wallet for this member
        dk.panos.promofacie.db.Wallet cardanoWallet = new dk.panos.promofacie.db.Wallet();
        cardanoWallet.setAddress("stake1uytestcardano");
        cardanoWallet.setDiscordId("discord-user-1");
        cardanoWallet.setChain(dk.panos.promofacie.db.Chain.CARDANO);

        dk.panos.promofacie.db.Wallet robinhoodWallet = new dk.panos.promofacie.db.Wallet();
        robinhoodWallet.setAddress("0x3a7907580002117eed3b6866723f32a9b5ddd463");
        robinhoodWallet.setDiscordId("discord-user-1");
        robinhoodWallet.setChain(dk.panos.promofacie.db.Chain.ROBINHOOD);

        doReturn(List.of(cardanoWallet, robinhoodWallet)).when(resource).getWalletsForDiscordIds(List.of("discord-user-1"));
        ...
        // Verify Robinhood emitter received ADD_POLICY for 0x2222... and REMOVE_POLICY for 0x1111...
        ArgumentCaptor<TrackingCommand> rhCaptor = ArgumentCaptor.forClass(TrackingCommand.class);
        verify(robinhoodEmitter, atLeast(2)).send(rhCaptor.capture());
        List<TrackingCommand> rhCommands = rhCaptor.getAllValues();

        assertTrue(rhCommands.stream().anyMatch(c -> c.action() == TrackingCommand.Action.ADD_POLICY && "0x2222222222222222222222222222222222222222".equals(c.policyId())));
        assertTrue(rhCommands.stream().anyMatch(c -> c.action() == TrackingCommand.Action.REMOVE_POLICY && "0x1111111111111111111111111111111111111111".equals(c.policyId())));
        // And verified that robinhood wallet was sent ADD_ADDRESS to Robinhood emitter
        assertTrue(rhCommands.stream().anyMatch(c -> c.action() == TrackingCommand.Action.ADD_ADDRESS && "0x3a7907580002117eed3b6866723f32a9b5ddd463".equals(c.stakeAddress())));

        // Cardano emitter should have received ADD_ADDRESS for the cardano wallet
        ArgumentCaptor<TrackingCommand> cardanoCaptor = ArgumentCaptor.forClass(TrackingCommand.class);
        verify(cardanoEmitter, atLeast(1)).send(cardanoCaptor.capture());
        List<TrackingCommand> cardanoCommands = cardanoCaptor.getAllValues();
        assertTrue(cardanoCommands.stream().anyMatch(c -> c.action() == TrackingCommand.Action.ADD_ADDRESS && "stake1uytestcardano".equals(c.stakeAddress())));
        // Neither ADD_POLICY nor REMOVE_POLICY for 0x contracts should go to Cardano emitter
        assertFalse(cardanoCommands.stream().anyMatch(c -> c.action() == TrackingCommand.Action.ADD_POLICY));
        assertFalse(cardanoCommands.stream().anyMatch(c -> c.action() == TrackingCommand.Action.REMOVE_POLICY));
    }
}
