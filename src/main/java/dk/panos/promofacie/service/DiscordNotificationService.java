    ...
    private void sendDiscordChannelNotification(String guildId, String channelId, TransactionMessage message) {
        try {
            ...
            if (isWayup) {
                if (message.policyId() != null && !message.policyId().isBlank()) {
                    String wayupUrl;
                    if (message.requestedAssetNameHex() != null && !message.requestedAssetNameHex().isBlank()) {
                        wayupUrl = "https://www.wayup.io/collection/" + message.policyId() + "/asset/" + message.requestedAssetNameHex();
                    } else {
                        wayupUrl = "https://www.wayup.io/collection/" + message.policyId();
                    }
                    ...
                    log.warn("[Discord] Platform is WayUp but missing policyId for txHash={}, cannot build WayUp button", message.txHash());
                }
            } else {
